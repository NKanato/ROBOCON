...
int counter = 0;  // used to control display output rate
...
class MPU6050 {
    ...
    char readByte(uint8_t address, uint8_t subAddress)
    {
        ...
        char data_write[1];
        data_write[0] = subAddress;
        reg_w = i2c.write(address, data_write, 1, 1); // no stop
        reg_r = i2c.read(address, data, 1, 0); 
        if(reg_w != 0){
            printf("failed writeing\n\r");
        }
        if(reg_r != 0){
    ...
    void LowPowerAccelOnly(void)
    {
        ...
        ThisThread::sleep_for(100);  // Add delay for accumulation of samples
    ...
    {  
        ...
        ThisThread::sleep_for(100); // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt  
    ...
    {  
        ...
        ThisThread::sleep_for(80); // accumulate 80 samples in 80 milliseconds = 960 bytes
        ...
        accel_bias[0] /= (int32_t) packet_count; // Normalize sums to get average counter biases
    ...
    void MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        ThisThread::sleep_for(250);  // Delay a while to let the device execute the self-test
    ...
    void SetUp_MPU6050(void)
    {
        ...
        if (whoami == 0x68){
            ...
            printf("%#x \n",  whoami);
            //MPU6050_Error = 1;
            //while(1) ; // Loop forever if communication doesn't happen// commented out8/11
        }
    }
